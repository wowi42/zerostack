...
| `alternate_screen`        | boolean | When `true` (default), the TUI uses the terminal's alternate screen and captures the mouse. When `false`, zerostack renders into the terminal's main buffer: new lines are appended to the native scrollback, native mouse selection works, and the terminal's own scrollbar/wheel scroll the history. The status bar and input area are rendered as ordinary lines at the bottom and scroll away with content rather than staying fixed. Experimental. |
