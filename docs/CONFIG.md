...
| `alternate_screen`        | boolean | When `true` (default), the TUI uses the terminal's alternate screen and captures the mouse. When `false`, zerostack paints the same fullscreen frame on the terminal's main buffer: the status bar and input area stay anchored at the bottom, native mouse selection and copy/paste work, and the final frame remains visible in the terminal history after the program exits. The terminal's scrollbar does not scroll through the live conversation history because the visible screen is redrawn each frame. Experimental. |
